...
public class SucursalServiceImpl implements SucursalService {
    ...
    @Override public SucursalResponseDTO crear(SucursalRequestDTO req) {
        Sucursal s = new Sucursal(null, req.getNombre(), req.getDireccion(), req.getDepartamento());
        return toDTO(repo.save(s));
    }
    @Override public SucursalResponseDTO obtenerPorId(Integer id) { return toDTO(buscar(id)); }
    @Override public List<SucursalResponseDTO> listarTodos() {
        return repo.findAll().stream().map(this::toDTO).collect(Collectors.toList());
    }
    @Override public SucursalResponseDTO actualizar(Integer id, SucursalRequestDTO req) {
        Sucursal s = buscar(id);
        s.setNombre(req.getNombre()); s.setDireccion(req.getDireccion()); s.setDepartamento(req.getDepartamento());
        return toDTO(repo.save(s));
    }
